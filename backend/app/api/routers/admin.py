...
async def get_audit_logs(current_user=Depends(get_current_user), session: AsyncSession = Depends(get_async_session)) -> List[AuditEventResponse]:
    return await AdminService(session).list_audit_logs()

@router.get("/security-events", response_model=List[SecurityEventResponse])
async def get_security_events(current_user=Depends(get_current_user), session: AsyncSession = Depends(get_async_session)) -> List[SecurityEventResponse]:
