...
Zancrypt uses **WebAuthn (FIDO2)**. This relies on hardware-bound authenticators like FaceID, TouchID, Windows Hello, or hardware security keys (like YubiKey). This guarantees high security and resists phishing. Logins are completely passwordless via passkeys, and account recovery is managed through a cryptographically hashed recovery key.
