...
Rather than relying on passwords sent over the network, Zancrypt anchors user identities exclusively to hardware biometrics:
1. **Registration**: The server generates challenges using the `fido2` Python library. The web prompts the browser's native `navigator.credentials.create()` for TouchID, FaceID, Windows Hello, or YubiKey authentication.
2. **Verification**: The client sends back a signed attestation, a `recovery_key`, and a `master_key_salt`.
3. **Storage**: The server cryptographically hashes the `recovery_key` using SHA-256 and `bcrypt` into `recovery_key_hash` for emergency account recovery, while `WebAuthnCredential` models store the public key and sign counts. All logins are strictly passwordless via passkeys. 
